...
- **Where a feature has regulatory exposure, ship the part that helps and hold the part that creates liability.** Founder-generalised 2026-08-10 from the M4 commission constraint (calculate and display, never file or pay, until the payroll advisor has spoken). This is the standing answer to the four open advisor items: build up to the liability line and stop, rather than waiting for advice or building through it.
- **Set the criteria for a decision point before the test, not after.** Founder directive 2026-08-10: criteria written afterwards get rationalised into a pass. Applies to pilots, benchmarks, validations, gates. Also define the failure mode that *hides* — the one that shows healthy metrics while the thing actually failed.
- **Free is a positioning decision, not an absence of billing.** Founder directive 2026-08-10: businesses given something free for months resist paying afterwards. Anything unpriced because we cannot yet charge is presented as a founding-member arrangement with the price stated up front.
